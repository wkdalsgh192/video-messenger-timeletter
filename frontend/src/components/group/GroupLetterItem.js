...
import { Chip, Dialog, Slide } from "@material-ui/core";
...
const GroupLetterItem = (props) => {
  ...
  const dDayCounter = () => {
    let dDay = toDate(props.letter.open_date);
    setInterval(function () {
      ...
      if (distance < 0) {
        setOpenInfo("오픈날짜 " + props.letter.open_date);
      } else {
        if (d > 0) {
          view = view + d + "일 ";
        }
  ...
  const getInfo = () => {
    if (props.letter.is_open) {
      ...
      return (
        <div style={{ fontSize: "20px", color: "#fff" }}> {closeInfo}</div>
      );
    }
  };
  ...
  return (
    <div onClick={handleClick}>
      ...
      <Dialog
        ...
        TransitionComponent={VideoTransition}
      >
...
export default GroupLetterItem;
